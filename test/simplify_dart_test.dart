    ...
    test('simplifies points correctly with the given tolerance', () {
      var pointsList = points
          .map((e) => Point()
            ...
            ..y = e[1])
          .toList();
      var simplifiedActual = simplify(pointsList, tolerance: 5);
      expect(simplifiedActual.length, simplified.length);

      for (var i = 0; i < simplifiedActual.length; i++) {
        var actPoint = simplifiedActual[i];
        var expPoint = simplified[i];
        expect(actPoint.x, expPoint[0]);
        expect(actPoint.y, expPoint[1]);
      }
    });
    test('just return the points if it has only one point', () {
      var simplifiedActual = simplify([
        Point()
          ..x = 1
          ..y = 2
      ]);
      expect(simplifiedActual.length, 1);
      expect(simplifiedActual[0].x, 1);
      expect(simplifiedActual[0].y, 2);
    });
    test('just return the points if it has no points', () {
      var simplifiedActual = simplify([]);
      expect(simplifiedActual.length, 0);
    });
  });
}
